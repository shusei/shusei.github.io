...
import { chatWithReceptionistService } from '../services/ai.service';

/**
 * Chat with AI Receptionist
 * POST /api/quests/receptionist/chat
 */
export const chatWithReceptionist = async (req: Request, res: Response) => {
    try {
        ...
        const reply = await chatWithReceptionistService(history || [], message);
        res.json(reply);
    } catch (error) {
        console.error('Error chatting with receptionist:', error);
